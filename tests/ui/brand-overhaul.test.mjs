...
for(const [path,hash] of Object.entries(baseline.scripts)) {
  ...
  if(path==='production-web/web/cloud-tournaments-shortcut.js'){
    for(const marker of [
      'nativeWebShell:true',
    ...
    ]) assert.ok(source.includes(marker),`intentional Web shell adapter contract missing: ${marker}`);
    continue;
  }
  assert.equal(sha(source),hash,`unchanged production adapter: ${path}`);
}
...
assert.ok(css.includes(':not(.cp-beta7-app-hidden):not([style*="display: none"]):not([style*="display:none"])'),
  'desktop compatibility selector must respect sign-in hiding and Close/Reopen');
assert.ok(css.includes('#appWindow.app-window-minimized {height:auto!important'),
  'minimized window action must remain reachable through the compatibility menu');
...
console.log(`UI preservation PASS: exact HTML baseline ${baseline.baseCommit}, ${scripts} inline scripts and protected adapters preserved; intentional Web shell adapter covered by explicit contracts.`);