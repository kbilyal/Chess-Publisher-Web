...
// one Cloud identity, refresh-free Arbiter unified SYNC, desktop-only result ACK.
...
assert.match(cloudApi, /exactName=rows\.filter\(row=>!row\?\.archivedAt&&normalizedName\(row\?\.name\)===targetName\)/, 'Legacy Cloud recovery compatibility marker must remain available until its migration is completed.');
...
assert.match(downloader, /"0F - 0F","-"/, 'Desktop result reconciliation must accept Clear result as a normal synchronized change.');
