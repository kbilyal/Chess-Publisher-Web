...
SOURCE="$REPO_ROOT/production-web"
DEST="${1:-$REPO_ROOT/dist}"

test -f "$SOURCE/index.html"
test -f "$SOURCE/CNAME"
test "$(cat "$SOURCE/CNAME")" = "web.chess-publisher.org"
SOURCE_DIR="$SOURCE" python - <<'PY'
from hashlib import sha256
from pathlib import Path
import os
import re

index = Path(os.environ['SOURCE_DIR'], 'index.html').read_bytes()
shell = re.sub(rb'<!-- cpProductionWeb .*?</body>', b'</body>', index, count=1, flags=re.S)
expected = 'f6427a3c2ade30ec2799bdbcbf69688a3247e969c6f03e82daf11901386f791a'
if sha256(shell).hexdigest() != expected:
  raise SystemExit('production-web/index.html is not the beta.6 desktop shell')
PY
...
for file in \
  ...
  webview/CloudWorkspaceAdapter.js; do
  test -f "$SOURCE/$file"
done
...
mkdir -p "$DEST"
cp -R "$SOURCE/." "$DEST/"
printf 'Materialized canonical production-web at %s\n' "$DEST"
#!/usr/bin/env bash
set -euo pipefail
...
ZIP="$TMP/desktop-web-source.zip"
...
DOWNLOAD_OK=0
if python -m gdown "https://drive.google.com/uc?id=${ZIP_ID}" -O "$ZIP"; then
  DOWNLOAD_OK=1
fi
...
if [[ "$DOWNLOAD_OK" -ne 1 || ! -s "$ZIP" ]]; then
  echo "ERROR: Could not download the exact desktop ChessPublisher source package." >&2
  exit 1
fi
...
Path(sys.argv[2]).write_bytes(source[:position] + bootstrap + source[position:])
PY

echo "${INDEX_SHA256}  ${DEST}/index.html" | sha256sum -c -
grep -q 'cpProductionWeb' "$DEST/index.html"
grep -q 'cpLinuxWebDev' "$DEST/index.html"
grep -q 'Chess-Publisher' "$DEST/index.html"
grep -q 'Online & Cloud' "$DEST/index.html"
grep -q 'Pull Changes' "$DEST/index.html"
grep -q 'Sync Now' "$DEST/index.html"
...
echo "Materialized exact desktop ChessPublisher production shell at: $DEST"
