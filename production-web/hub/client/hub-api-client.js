(function(root,factory){
  ...
  const DEFAULT_CLIENT_VERSION="1.06.00-beta.1";
  const SECRET_KEY_RE=/(authorization|token|secret|password|beta[_-]?key|manage[_-]?key)/i;
  const MAX_REGULATIONS_FILE_BYTES=20*1024*1024;
  ...
  function createClient(options={}){
    ...
    async function request(path,init={}){
      ...
      if(body!==undefined&&body!==null&&typeof body!=="string"&&!isFormData&&!isBlob&&!isArrayBuffer){
      ...
      if(!response.ok){
        const code=typeof payload?.error==="string"?text(payload.error):text(payload?.error?.code)||`http_${response.status}`;
    ...
    return Object.freeze({
      baseUrl,
      clientVersion,
      health(){return request("/api/v1/health",{method:"GET"});},
      organizerMe(organizerToken){return request("/api/v1/organizer/me",{method:"GET",headers:organizerBearer(organizerToken)});},
      listOrganizerTournaments(organizerToken){return request("/api/v1/organizer/tournaments",{method:"GET",headers:organizerBearer(organizerToken)});},
      createOrganizerTournament(organizerToken,metadata){
        return request("/api/v1/organizer/tournaments",{method:"POST",headers:organizerBearer(organizerToken),body:createTournamentPayload(metadata)});
      },
      ...
      uploadRegulationsFile(args){
        if(!args||typeof args!=="object") throw new Error("Regulations upload arguments are required.");
        const tournamentId=encodeURIComponent(required(args.tournamentId,"Hub tournament ID"));
        const file=args.file;
        if(!file||typeof file!=="object") throw new Error("Choose a regulations file first.");
        const fileName=required(file.name,"Regulations filename");
        const size=Number(file.size);
        if(!Number.isFinite(size)||size<0) throw new Error("Regulations file size is invalid.");
        if(size>MAX_REGULATIONS_FILE_BYTES) throw new Error("Regulations file exceeds the 20 MiB limit.");
        if(typeof FormData==="undefined") throw new Error("This WebView does not support file uploads.");
        const form=new FormData();
        // Passing the original File object and filename preserves arbitrary binary
        // content and Unicode/Cyrillic filenames in Chromium/WebView2 multipart.
        form.append("file",file,fileName);
        return request(`/api/v1/tournaments/${tournamentId}/regulations-file`,{
          method:"PUT",
      ...
      deleteTournament(args){
        ...
        return request(`/api/v1/tournaments/${tournamentId}`,{method:"DELETE",headers:managedHeaders(args.organizerToken,args.manageToken,{"X-Confirm-Delete":publicSlug})});
      },
      restoreTournament(args){
        if(!args||typeof args!=="object") throw new Error("Hub restore arguments are required.");
        const tournamentId=encodeURIComponent(required(args.tournamentId,"Hub tournament ID"));
        return request(`/api/v1/tournaments/${tournamentId}/restore`,{method:"POST",headers:managedHeaders(args.organizerToken,args.manageToken)});
      },
      listPublicTournaments(){return request("/api/v1/public/tournaments",{method:"GET"});},
      getPublicTournament(idOrSlug){return request(`/api/v1/public/tournaments/${encodeURIComponent(required(idOrSlug,"Hub public tournament identifier"))}`,{method:"GET"});}
    });
  }

  return Object.freeze({DEFAULT_BASE_URL,DEFAULT_CLIENT_VERSION,MAX_REGULATIONS_FILE_BYTES,HubApiError,createClient,createTournamentPayload,publicLinkRecord,redactSensitive});
});
