...
const expectedShell = "9df4775d0480555bfdb4b22065ff1708ff8c96a0f4230c6e378627f0b5d118c1";
...
assert(index.includes("function cpIsCloudOnlyWeb()"), "hosted Web does not identify its cloud-only save mode");
assert(index.includes("const saveToCloud=window.cpCloudSaveCurrent;"), "hosted Web Save does not delegate to Private Cloud");
assert(index.includes("Chess-Publisher Web saves tournament changes to Private Cloud."), "hosted Web still offers local Save As behavior");
assert(index.includes('id="cpCloudOnlySaveUi"'), "hosted Web Save/Autosave controls are not labelled as Cloud storage");
...
assert(cloudAdapter.includes("async function saveCurrentToCloud"), "cloud-only Web save endpoint is missing");
assert(cloudAdapter.includes("window.cpCloudSaveCurrent=saveCurrentToCloud;"), "cloud-only Web save endpoint is not exported");
assert(cloudAdapter.includes("!quiet&&!isCloudOnlyWeb()&&typeof fileSaveTournament"), "hosted Web sync still attempts a local file save");
