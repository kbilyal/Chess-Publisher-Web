...
class InMemoryRevisionStore {
  ...
  read() { return { revision: this.revision, snapshot: clone(this.snapshot) }; }
...
async function remoteTournament(store: InMemoryRevisionStore) {
  ...
  return { ...remote, tournament: parsed.tournament, fingerprint: await fingerprintTournament(parsed.tournament) };
}
...
async function testDesktopWebRoundtripAndConflicts() {
  ...
  assert.equal(decideAutomaticSync(await fingerprintTournament(web), web.cloud.baseFingerprint, remote.fingerprint), 'remote-available');
  web = preserveInstallationLocalFields(remote.tournament, web, {
    cloudTournamentId: CLOUD_ID, baseRevision: remote.revision, baseFingerprint: remote.fingerprint
  });
  ...
  remote = await remoteTournament(store);
  assert.equal(decideAutomaticSync(await fingerprintTournament(desktop), desktop.cloud.baseFingerprint, remote.fingerprint), 'remote-available');
  ...
  assert.equal(decideAutomaticSync(await fingerprintTournament(web), web.cloud.baseFingerprint, remote.fingerprint), 'conflict');
  await expectControlledRevisionConflict(() => store.put(web.cloud.baseRevision, buildPrivateSnapshot(NAME, web)), 4);
  ...
  remote = await remoteTournament(store);
  assert.equal(decideAutomaticSync(await fingerprintTournament(webB), webB.cloud.baseFingerprint, remote.fingerprint), 'conflict');
  await expectControlledRevisionConflict(() => store.put(webB.cloud.baseRevision, buildPrivateSnapshot(NAME, webB)), 5);
  ...
  assert.deepEqual(Object.keys(uploaded.cloud).sort(), ['cloudTournamentId', 'internalId', 'schemaVersion']);
  assert.equal(uploaded.cloud.internalId, INTERNAL_ID);
  assert.equal(uploaded.cloud.cloudTournamentId, CLOUD_ID);
...
async function testPullChangesConcurrency() {
  ...
  const activeSync = activeCoordinator.enqueue(async () => {
    order.push('sync-start'); await syncBarrier; order.push('sync-end');
  });
